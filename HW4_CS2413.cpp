...
//print SID function
void List::PrtSID() {
    Node* temp = head; //create a new node and assign the head to it 
    while (temp != NULL) {
        cout << temp->Get_SID() << " "; //print out student ID
        temp = temp->Get_Pnext(); //move to the next node in the linked list
    }
    cout << endl;
}
...
// and "Lsize+1" means adding to tail.
    ...
    int size = Lsize(); //size of linked list
    if (idx < 1 || idx > size + 1) {
        return -1;
    }
    ...
    if (idx == 1) {
        ...
        head = p; //update head pointer to point to new node p
    }
    else {
        Node* temp = head;

        for (int i = 1; i < idx - 1; i++) {
            temp = temp->Get_Pnext(); //travel to the node just before where the new node will be inserted

        }
        p->Set_Pnext(temp->Get_Pnext()); // Point the new node to the node currently at position idx
        temp->Set_Pnext(p); // Point the node at position idx-1 to the new node
    }
...
// and "Lsize" means removing tail.
    ...
    if (idx == 1) {
        ...
        for (int i = 1; i < idx - 1; i++) {
            temp = temp->Get_Pnext(); //traverse to node just before the node that is going to be removed
        }

        Node* toRemove = temp->Get_Pnext(); //node at position idx
        temp->Set_Pnext(toRemove->Get_Pnext()); //whatever the node that is removed was pointing to, point node (before removed node) to it
        delete toRemove;
    }
...
// running Reverse we should get 642.
    ...
    Node* next = NULL;  //node that the current node is pointing to

    //loop through all nodes of the linked list
    while (current != NULL) {
        next = current->Get_Pnext(); //set the next node to whatever the current node is pointing to 
        current->Set_Pnext(previous); //point the current node to the previous node (since it is reversed linked list)
        previous = current; //increment and update previous pointer to the current node 
        current = next; //then move to the next node in the linked list
    }

    head = previous; //update head pointer to first node in reverse linked list
}
